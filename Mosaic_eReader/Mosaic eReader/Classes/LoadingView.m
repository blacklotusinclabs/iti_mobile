...
+ (id)loadingViewInView:(UIView *)aSuperview 
       ...
       withProgressView: (BOOL)hasProgressView;
{
    ...
    if(hasProgressView)
    {
        loadingView.progressView = [[UIProgressView alloc] initWithProgressViewStyle: UIProgressViewStyleDefault];            
        [loadingView addSubview:loadingView.progressView];
        loadingView.progressView.autoresizingMask =
        UIViewAutoresizingFlexibleLeftMargin |
        UIViewAutoresizingFlexibleRightMargin |
        UIViewAutoresizingFlexibleTopMargin |
        UIViewAutoresizingFlexibleBottomMargin;
        [loadingView.progressView release];
    }

	CGFloat totalHeight = 0;
    if(hasProgressView)
    {
		totalHeight = loadingLabel.frame.size.height + loadingView.progressView.frame.size.height;
    }
    else
    {
		totalHeight = loadingLabel.frame.size.height + activityIndicatorView.frame.size.height;        
    }
    ...
    else
    {
        ...
        progressViewRect.origin.y = loadingLabel.frame.origin.y + loadingLabel.frame.size.height;
...
+ (id) loadingViewInView:(UIView *)aSuperview withProgressView:(BOOL)hasProgressView
{
    return [self loadingViewInView:aSuperview 
                          withText:@"Loading..." 
                  withProgressView:hasProgressView];
}

+ (id) loadingViewInView:(UIView *)aSuperview 
                withText:(NSString *)titleText
{
    return [self loadingViewInView:aSuperview withText:titleText withProgressView:NO];
}

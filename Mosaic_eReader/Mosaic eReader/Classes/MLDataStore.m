...
- (void) clearStore
{
    ...
    MosaicEReaderAppDelegate *delegate = (MosaicEReaderAppDelegate *)[[UIApplication sharedApplication] delegate];
    
    // Delete all entities...
    [delegate deleteAllObjects:@"Book"];                                      
    ...
    // blank...
    userBooks = nil; // [[NSMutableDictionary alloc] initWithCapacity: 10];
	availableBooks = nil; // [[NSMutableDictionary alloc] initWithCapacity: 10];
	accessibleBooks = nil; // [[NSMutableArray alloc] initWithCapacity: 10];
	notAvailableBooks = nil; // [[NSMutableDictionary alloc] initWithCapacity: 10];
...
- (void) clearStoreExceptUserBooks
{
    ...
    // Delete all entities...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"collectionId != %@",USER_BOOKS_ID];
    [delegate deleteAllObjects:@"Book" 
    ...
    for(NSString *bookId in [userBooks allKeys])
    {
        ...
        Book *aBook = [array objectAtIndex:0];
        [aBook setCollectionId:USER_BOOKS_ID];
        [self saveContext];
        [lock unlock];
    }
...
- (void) removeBook:(NSString *)bookId
{
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:
                              @"bookId like %@ and collectionId = %@", bookId, USER_BOOKS_ID];
    [request setPredicate:predicate];
...
- (void) removeAvailableBook: (NSString *)bookId
{
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:
                              @"bookId like %@ and collectionId = %@", bookId, AVAILABLE_BOOKS_ID];
    [request setPredicate:predicate];
...
- (NSMutableArray *) allDownloadedBooks
{
    ...
    NSArray *sortedArray = [[userBooks allValues] sortedArrayUsingFunction:stringSort
                                                                   context:nil];
    NSMutableArray *resultArray = [sortedArray mutableCopy];
...
- (NSMutableArray *) allAvailableBooks
{
    ...
    NSArray *sortedArray = [[availableBooks allValues] sortedArrayUsingFunction:stringSort
                                                                   context:nil];
    NSMutableArray *resultArray = [sortedArray mutableCopy];
    return [resultArray autorelease];
}
...
- (NSMutableArray *) allNotAvailableBooks
{
    ...
    NSArray *sortedArray = [[notAvailableBooks allValues] sortedArrayUsingFunction:stringSort
                                                                        context:nil];
    NSMutableArray *resultArray = [sortedArray mutableCopy];
    return [resultArray autorelease];
}
